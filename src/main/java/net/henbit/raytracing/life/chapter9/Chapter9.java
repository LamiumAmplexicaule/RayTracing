package net.henbit.raytracing.life.chapter9;
...
public class Chapter9
{
    ...
    private Vector3 rayColor(final Ray r, final Vector3 background, Hittable world, int depth)
    {
        ...
        Vector3 onLight = new Vector3(randomDouble(213, 343), 554, randomDouble(227, 332));
        Vector3 toLight = onLight.subtract(hitRecord.point);
        ...
        double lightCosine = abs(toLight.y());
        if (lightCosine < 0.000001)
            return emitted;

        pdf.copy(distanceSquared / (lightCosine * lightArea));
        scattered = new Ray(hitRecord.point, toLight, r.time());

        return emitted.add(albedo)
                .multiply(hitRecord.material.scatteringPDF(r, hitRecord, scattered))
                .multiply(rayColor(scattered, background, world, depth - 1)).divide(pdf.getValue());
    }
